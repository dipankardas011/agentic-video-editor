...
const FORCED_MP4_ENCODERS = new Set(['h264_nvenc', 'h264_qsv', 'libx264']);
let _loggedMp4Encoder = null;

async function getMp4VideoCodecArgs(quality) {
  const forced = (process.env.FFMPEG_MP4_ENCODER || '').trim();
  const encoder = FORCED_MP4_ENCODERS.has(forced) ? forced : 'libx264';
  if (_loggedMp4Encoder !== `${encoder}:${forced ? 'forced' : 'default'}`) {
    console.log('[ffmpeg encoder]', encoder, forced ? '(forced)' : '(default)');
    _loggedMp4Encoder = `${encoder}:${forced ? 'forced' : 'default'}`;
  }
  if (encoder === 'h264_nvenc') {
    return ['-c:v', 'h264_nvenc', '-preset', 'p5', '-cq', String(quality), '-b:v', '0'];
  }
  if (encoder === 'h264_qsv') {
    return ['-c:v', 'h264_qsv', '-global_quality', String(quality), '-look_ahead', '0'];
  }
  return ['-c:v', 'libx264', '-preset', 'medium', '-crf', String(quality)];
}

async function buildCodecArgs(format, quality, speed, filters) {
  if (format === 'gif') {
  ...
  // mp4
  const args = [...await getMp4VideoCodecArgs(quality), '-c:a', 'aac', '-b:a', '192k'];
  if (filters.length) args.push('-vf', filters.join(','));
  if (speed !== 1) args.push('-af', `atempo=${Math.max(0.5, Math.min(2, speed))}`);
  return args;
}
...
async function encodeItem(item, idx, { format, quality, speed, filters }, onProgress) {
  ...
  if (isImg) {
    ...
    if (format === 'webm') {
      ...
      args.push(...await getMp4VideoCodecArgs(quality), '-c:a', 'aac', '-b:a', '192k');
    }
...
async function renderTimeline(segments, { format = 'mp4', resolution = 'fhd', quality = 23, speed = 1 } = {}, onProgress) {
  ...
  timelineEntries.forEach((entry, idx) => {
    ...
    if (entry.trackType === 'video' && mediaInfo.hasVideo) {
      ...
      // LUT must be applied before alpha conversion
      if (entry.lut) {
        const safeLutPath = entry.lut.replace(/\\/g, '\\\\').replace(/:/g, '\\:');
        videoFilters.push(`lut3d=${safeLutPath}`);
      }
  ...
  if (timelineExt === 'webm') {
    ...
    args.push(...await getMp4VideoCodecArgs(quality), '-pix_fmt', 'yuv420p', '-c:a', 'aac', '-b:a', '192k');
  }
  ...
  const gifCodec = await buildCodecArgs('gif', quality, 1, []);
...
async function renderItems(items, { format = 'mp4', resolution = 'fhd', quality = 23, speed = 1 } = {}, onProgress) {
  ...
  if (items.length === 1) {
    ...
    if (canCopy) {
      ...
      const codec = await buildCodecArgs('gif', quality, speed, filters);
...
async function applyTextOverlays(inputPath, ext, textOverlays) {
  ...
  // Write each overlay text to a temp file so we avoid all quoting/comma escaping issues.
  // drawtext's textfile= option reads the text from disk, bypassing filter-string parsing entirely.
  const tmpFiles = [];
  const filters = textOverlays.map(ov => {
    const tf = path.join(os.tmpdir(), `sub_${Date.now()}_${Math.random().toString(36).slice(2)}.txt`);
    writeFileSync(tf, ov.text);
    tmpFiles.push(tf);
    ...
    const enable = `between(t,${ffnum(ov.startTime)},${ffnum(ov.startTime + ov.duration)})`;
    return `drawtext=textfile='${tf}':fontsize=${size}:fontcolor=${color}:x=${x}:y=${y}:enable='${enable}':shadowx=2:shadowy=2:shadowcolor=black@0.7`;
  });
  ...
  if (ext === 'webm') {
    ...
    args.push(...await getMp4VideoCodecArgs(23), '-c:a', 'copy');
  }
  args.push(output);
